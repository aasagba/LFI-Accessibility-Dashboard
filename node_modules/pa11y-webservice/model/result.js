...
module.exports = function (app, callback) {
	app.db.collection('results', function (err, collection) {
		...
		var model = {
			...
			// Test to get only fields needed for homepage
			_getFilteredLight: function (opts, callback) {
				opts = model._defaultFilterOpts(opts);
				var filter = {
					/*date: {
						$lt: opts.to,
				...
				collection
					.find(filter,{_id:1,count:1,task:1,ignore:1,date:1})
					.sort({date: -1})
					.limit(opts.limit || 0)
					.toArray(function (err, results) {
						if (err) {
			...
			getByTaskId: function (id, opts, callback) {
				//console.log("in getByTaskId");
				//console.log("id: " + id);
				//console.log("opts: " + opts);
				opts.task = id;
				//console.log("opts.tasK: " + JSON.stringify(opts.task));
				model._getFiltered(opts, callback);
			},

			// Get count results for a single task for homepage
			getLightByTaskId: function (id, opts, callback) {
				//console.log("in getByTaskId");
				//console.log("id: " + id);
				//console.log("opts: " + opts);
				opts.task = id;
				//console.log("opts.tasK: " + JSON.stringify(opts.task));
				model._getFilteredLight(opts, callback);
			},
